...
def processNewDF(new_df):
    ...
    """
    prev_df = grabFromCassandra() # Data to be updated
    #overwriteDB(cassandra['worker1'])
...
def grabFromCassandra():
    """
    Connect to local Cassandra DB (de-ny-drew2), or Cassandra DB
    passed in via spark-submit in the --conf flag.
...
#
#def overwriteDB(seed_ip):
#    cluster = Cluster()
#    cluster = Cluster(seed_ip) # Only need 1 private ip, it will auto discover remaining nodes in cluster
#    session = cluster.connect()
#    session.execute("TRUNCATE advertise.usersearches")
#    cluster.shutdown()
#    
#
